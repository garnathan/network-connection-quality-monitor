# Home Broadband Upload Performance Report

**Date:** 2026-04-20
**External IP:** 80.233.43.68 (ISP: AS allocation)
**Test host:** MacBook Pro, macOS 25.3.0, connected via WiFi (en0)
**Summary verdict:** Sustained HTTPS uploads collapse at ~5% of advertised uplink
bandwidth and frequently terminate with `connection reset by peer` before
completing. The behavior is **provider-path-agnostic** (Oracle and AWS both
exhibit identical failure modes) and consistent with **bufferbloat on the
upstream path** plus probable **sustained-upload throttling**.

---

## 1. Baseline bandwidth (Apple `networkQuality` — gold-standard RFC 9097 test)
